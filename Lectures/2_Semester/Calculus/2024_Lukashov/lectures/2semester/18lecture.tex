...
\begin{note}
	Всюду в этом параграфе рассматриваются лишь подмножества $K_I$, определённого в предыдущем параграфе. И соответствующая им алгебра.
\end{note}

\begin{definition}
	\textit{Внешней мерой Жордана} множества $A$ называется величина
	\[
		\mu^*_\gJ(A) = \inf_{A \subset \bigcup\limits_{i = 1}^N M_i} \ \left\{\suml_{i = 1}^N |M_i|\right\}
	\]
...
\begin{definition}
	...
	\]
	где $\inf$ берётся по всем покрытиям $A$ не более чем счётным числом элементарных множеств.
\end{definition}
...
\begin{theorem} (Основные свойства внешних мер)
	\begin{enumerate}
		...
		\item Если $A \subset \bigcup\limits_{i = 1}^N A_i$, то
		\[
			\upjlm(A) \le \suml_{i = 1}^N \upjlm(A_i)
		\]
...
\begin{proof}~
	\begin{enumerate}
		\item \begin{enumerate}
			\item \(A \subset A\text{- тривиальное покрытие, тогда по определению} \Lora \upjlm(A) \le |A|\)
			
			\item \(A \subset \bigcup_{i = 1}^N M_i \Lora |A| \le \sum_{i = 1}^r |M_i| \Lora |A| \le \upjm(A)\)
		...
		\[
			\upjm(A_i) = \inf_{A_i \subset \bigcup_{j = 1}^{N_i} M_{i, j}} \left(\suml_{j = 1}^{N_i} |M_{i, j}|\right)
		\]
		Это означает, что
		\[
			(\forall \eps > 0)\ (\exists \{M_{i, j}\}_{j = 1}^{N_i}) \emb \suml_{j = 1}^{N_i} |M_{i, j}| < \upjm(A_i) + \frac{\eps}{2^i}
		\]
		Так как $A_i$ вложены в объединения $M_{i, j}$, то верен следующий факт:
		\[
			A \subset \bigcup_{i = 1}^N \bigcup_{j = 1}^{N_i} M_{i, j} \Longrightarrow (\forall \eps > 0) \emb  \upjm(A) \le \suml_{i = 1}^N \suml_{j = 1}^{N_i} |M_{i, j}| < \suml_{i = 1}^N \upjm(A_i) + \eps
		\]
		Для $\mu^*$ всё аналогично, просто нужно в $N_i$ поменять на бесконечности.
		...
		\[
			(\forall \eps > 0)\ (\exists \{M_{i, j}\}_{j = 1}^\infty) \emb \suml_{j = 1}^\infty |M_{i, j}| < \mu^*(A_i) + \frac{\eps}{2^i}
		\]
		Теперь счётно покрываем $A$:
		\[
			A \subset \bigcup_{i = 1}^\infty \bigcup_{j = 1}^\infty M_{i, j} \Longrightarrow (\forall \eps > 0) \emb \mu^*(A) \le \suml_{i = 1}^\infty \suml_{j = 1}^\infty |M_{i, j}| \le \suml_{i = 1}^\infty \mu^*(A_i) + \eps
		\]
	\end{enumerate}
\end{proof}
...
\begin{definition}
	Для $A \subset K_I$ \textit{внутренней мерой Лебега (Жордана)} называется величина
	\[
		\downjlm(A) := 1 - \upjlm(A')
	\]
	где $A' = K_I \bs A$
...
\begin{proof}
	...
	Тогда
	\[
		\downjlm(A) = 1 - \upjlm(A')\le \upjlm(A) + \upjlm(A') - \upjlm(A') = 
		 \upjlm(A)
	\]
\end{proof}
...
\begin{note}\textit{(Общие сведения)}
	\begin{itemize}
		\item Для удобства $A':= K_I\bs A$
		\item $\forall$ элементарное множество $M$ измеримо по Лебегу (Жордану), причём $\jlm(M) = |M|.\quad (M\subset M)$
		\item $\forall$ измеримое по Жордану множество $A$ измеримо по Лебегу, причём $\jm(A)=\mu(A)$.
	\end{itemize}
\end{note}
...
\begin{theorem} (Критерий измеримости)
	...
	\[
		(\forall \eps > 0)\ (\exists M_\eps) : \upjlm(A \tr M_\eps) < \eps
	\]
...
\begin{proof}~
	\begin{itemize}
		...
		\begin{itemize}
			\item \((\forall \eps > 0)\ (\exists \{P_i\}) : \suml_i |P_i| < \upjlm(A) + \eps\)
			
			\item \((\forall \eps > 0)\ (\exists \{Q_j\}) : \suml_j |Q_j| < \upjlm(A') + \eps\)
		\end{itemize}
		Зафиксируем $\eps > 0$. Сходимость последовательности частичных сумм позволяет нам ограничить остаток (это нужно для Лебега):
		\[
			(\exists r) : \suml_{i = r + 1}^\infty |P_i| < \eps; \quad (\exists s) : \suml_{j = s + 1}^\infty |Q_j| < \eps
		\]
...
\begin{proof}
	...
	\begin{itemize}
		...
		\[
			(\forall \eps > 0)\ (\exists M_\eps, N_\eps) : \upjlm(A \tr M_\eps) < \eps, \upjlm(B \tr N_\eps) < \eps
		\]

...
\begin{proof}
	...
	\begin{itemize}
		...
		\[
			(\forall \eps > 0)\ (\exists M_{1, \eps}, M_{2, \eps}) : \jlm(A_1 \tr M_{1, \eps}) < \eps,\ \ \jlm(A_2 \tr M_{2, \eps}) < \eps
		\]
		...
		\[
			(\forall \eps > 0) \emb\jlm(A_1) + \jlm(A_2) < \jlm(A_1 \sqcup A_2) + 6\eps
		\]
...
\begin{proof}
	...
	\[
		(\forall m \in \N) \emb \bscup_{i = 1}^m A_i \subset K_I \Ra \sum_{i = 1}^m \mu(A_i) \le 1
	\]
	то имеет место неравенство $0 \le \sum_{i = 1}^\infty \mu(A_i)\le 1$ и ряд сходится. Это даёт нам возможность оценить его конец:
	\[
		(\forall \eps > 0)\ (\exists r \in \N) : \sum_{i = r + 1}^\infty \mu(A_i) < \frac{\eps}{2}
	\]
	Плюс отметим следующий факт, следующий из измеримости множеств по Лебегу:
	\[
		(\forall i \in \N)\ (\forall \eps > 0)\ (\exists M_{i, \eps}) : \mu(A_i \tr M_{i, \eps}) < \frac{\eps}{2 \cdot r}
	\]
	...
	\begin{multline*}
		...
		\\
		\subset \bscup_{i = 1}^r (A_i \bs M_{i, \eps}) \cup \left(\bscup_{i = r + 1}^\infty A_i\right) \cup \bscup_{i = 1}^r (A_i \bs M_{i, \eps}) \subset \bigcup_{i = 1}^r (A_i \tr M_{i, \eps}) \cup \left(\bscup_{i = r + 1}^\infty A_i\right)
	\end{multline*}
...
\begin{definition}
	\textit{$\sigma$-алгебра} множеств (счётная алгебра множеств) - такая алгебра множеств, что объединение любой счётной совокупности множеств также принадлежит ей (алгебре).	
\end{definition}

\begin{corollary}
	Последняя теорема означает, что совокупность измеримых по Лебегу подмножеств $K_I$ образует \textit{$\sigma$-алгебру} множеств.
\end{corollary}

\begin{theorem} ($\sigma$-аддитивность меры Лебега)
	Если $\{A_i\}_{i = 1}^\infty$ - измеримые по Лебегу не пересекающиеся подмножества $K_I$, то
	\[
...
\begin{note}
	По сути вышесказанное значит, что для <<возрастающая>> (в смысле включения) последовательность множеств такова, что предел её мер равен мере её предела. Такая вот типо непрерывность меры Лебега. В связи с этим хотелось бы как-то формализовать понятие предела последовательности для множеств. Об этом далее. 	
\end{note}
\begin{definition}
...
\begin{definition}
	Пределы для множеств введём следующим образом:
	\begin{itemize}
		\item Если $\forall i \in \N\ \ A_i \subset A_{i + 1}$ (аналог невозрастающей последовательности), то
		\[
			\liml_{i \to \infty} A_i := \sup \{A_i\} = \bigcup_{i = 1}^\infty A_i
		\]
		
		\item Если $\forall i \in \N\ \ A_i \supset A_{i + 1}$ (аналог невозрастающей последовательности), то
		\[
			\liml_{i \to \infty} A_i := \inf \{A_i\} = \bigcap_{i = 1}^\infty A_i
		\]
		
		\item Верхний предел определим через эквивалентное свойство (здесь под пределом подразумевается предел невозрастающей последовательности, определённый выше):
		\[
		...
		\item Отсюда можно дать определение предела в общем случае как общее значение нижнего и верхнего предела, если они равны:
		\[
			\liml_{i \to \infty} A_i = \varliminf_{i \to \infty} A_i = \varlimsup_{i \to \infty} A_i
		\]
		Несложно убедиться, что определения предела неубывающей и невозрастающей последовательности согласуются с этим определением.
	\end{itemize}
\end{definition}
...
\begin{proof}
	...
	\[
		\mu(A_1) - \mu\left(\bigcap_{i = 1}^\infty A_i\right) = \mu\left(\bigcup_{i = 1}^\infty B_i\right) = \liml_{i \to \infty} \mu(B_i) = \mu(A_1) - \liml_{i \to \infty} \mu(A_i)
	\]
	Значит
	\[
	\liml_{i \to \infty} \mu(A_i) = \mu\left(\bigcap_{i = 1}^\infty A_i\right)
	\]
\end{proof}
...
\begin{proof}
	...
	\[
		\mu(\liml_{i \to \infty} \sup_{k \ge i} \{A_k\}) = \liml_{i \to \infty} \mu(\sup_{k \ge i} \{A_k\}) = \liml_{i \to \infty} \mu\left(\bigcup_{k = i}^\infty A_k\right) \ge \liml_{i \to \infty} \sup_{k \ge i} \mu(A_k) = 		\varlimsup_{i \to \infty} \mu(A_i)
	\]
\end{proof}
...
\begin{proof}
	...
	\[
		\mu(\varliminf_{i \to \infty} A_i) = \mu(\liml_{i \to \infty} \inf_{k \ge i} \{A_k\}) = \liml_{i \to \infty} \mu(\inf_{k \ge i} \{A_k\}) = \liml_{i \to \infty} \mu\left(\bigcap_{k = 1}^\infty A_k\right) \le \liml_{i \to \infty} \inf_{k \ge i} \mu(A_k) = \varliminf_{i \to \infty} \mu(A_i)
	\]
\end{proof}
...
\begin{proof} (теоремы \ref{4lemth})
	...
	\[
		\varlimsup_{i \to \infty} \mu(A_i) \le \mu(\varlimsup_{i \to \infty} A_i) = \mu(A) = \mu(\varliminf_{i \to \infty} A_i) \le \varliminf_{i \to \infty} \mu(A_i) \le \varlimsup_{i \to \infty} \mu(A_i)
	\]
	Что даёт равенство во всей цепочке, откуда тривиально следует искомое утверждение.
\end{proof}

\begin{definition}	\textit{(Множество измеримое по Жордану (Лебегу)   (общее определение)})
	Всё это время мы жили внутри куба $K_I$. Однако, пришло время выйти за его рамки: замостим всё пространство параллельными сдвигами $K_I$. Тогда, для любого множества $A$ мы можем определить измеримость и саму меру следующим образом.\\
	Множество $A$ измеримо по Жордану (Лебегу), если \[
	(\forall \vv{m} \in \Z^n ) \emb A) \cap (K_I + \vv{m})\text{ измеримо по Жордану (Лебегу).}
	\]
	Мерой такого множества будет 
	\[
		\jlm(A) := \sum_{\vv{m} \in \Z^n} \jlm\big((\vv{m} + K_I) \cap A\big)
	\]
	где $\vv{m} + K_I$ нужно воспринимать как сумму Минковского (каждая точка $K_I$ - это вектор. Новые точки получаются прибавлением $\vv{m}$ к этим векторам). Теперь, построив аналогичную теорию внутри каждого кубика, можем посчитать сумму мер кусочков множества, попавших в каждый из них. (Да, у нас повторяются взятые точки на границах кубов, но в силу того, что мера, как Жордана, так и Лебега у этих границ в целом $0$, то это не имеет значения).
	\\
	При этом обычно, когда говорят об измеримости по Жордану, берут только ограниченные множества. А вот для Лебега это не так, поэтому, у нас впервые сейчас формально возникнут множества с мерой $+\infty$ (когда ряд из определения расходится). Это впрочем никак не мешает построению теории.
\end{definition}

\begin{note}
	Отдельно отметим, что все свойства мер Лебега и Жордана остаются верными для любых множеств из $\R^n$ за исключением непрерывности:
	
	Рассмотрим $A_n = [n; +\infty)$. Для этой последовательности ломается лемма \ref{justLemma}, ибо мера $A_1$ - бесконечность. Для её (непрерывности) выполнения достаточно рассматривать множества, являющиеся подмножествами множества ограниченной меры.
\end{note}

\begin{note} (Про меру Жордана)
	Для ограниченных множеств можно заметить, что:
	\begin{itemize}
		\item $\upjm{A}$ = $\inf\limits_{A \subset M} |M|$, M - элементарные
		\item $\downjm{A} = \sup\limits_{M \subset A}|M| $, M - элементарные
	\end{itemize}
	Под элементарными множествами здесь будут подразумеваться конечные объединения брусьев, лежащих в некотором большом кубе, накрывающем $A$ (оно ограничено, поэтому так можно). \\
	Ранее мы ввели иное определение для аналогичности с мерой Лебега, что же до аналога этого определения у Лебега, для его введения нам понадобятся не элементарные множества, а кое-что посложнее, о чём ниже.
\end{note}
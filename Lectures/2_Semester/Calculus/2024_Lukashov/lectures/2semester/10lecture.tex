\textcolor{red}{ПРОПУЩЕННАЯ ЛЕКЦИЯ НАЧАЛО}
...
\begin{theorem} (Критерий Коши сходимости несобственного интеграла Римана)
	...
	\[
		\int_a^bf(x)dx \Longleftrightarrow (\forall \eps > 0)\ (\exists B \in (a;b)) : (\forall B_1, B_2,\ B < B_1 < B_2 < b)\emb \left | \int_{B_1}^{B_2}f(x)dx\right | < \eps
	\]
\end{theorem}
...
\begin{corollary}
	Если $f, g \in R[a;\tilde{b}],\ \forall \tilde{b},\ a < \tilde{b} < b$ и $f(x) \geq 0$, то
	\begin{multline*}
...
\begin{definition}
	Если $\int_a^b f(x)dx$ сходится, но не сходится абсолютно, то говорят, что он \textit{сходится условно}.
\end{definition}
...
\begin{theorem}(Признак Дирихле)
	...
	\begin{enumerate}
		...
		\item $g$ монотонна на $[a;b)$ и бесконечно малая при $x \to b - 0(+\infty)$
	\end{enumerate}
	Тогда $\int_a^b f(x)g(x)dx$ сходится.
\end{theorem}

\begin{proof}
	Рассмотрим $\forall a < B_1 < B_2 < b$. По формуле второй теореме о среднем:
	\[

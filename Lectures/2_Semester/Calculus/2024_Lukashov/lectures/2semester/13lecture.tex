...
\begin{proof}
	...
	\begin{itemize}
		...
		\[
			(\forall \eps > 0)\ (\exists m \in \N) : \suml_{k = m + 1}^\infty |a_k| < \frac{\eps}{2 M_2}
		\]
		...
		\[
			(\forall \eps > 0)\ (\exists N \in \N) : (\forall n > N) \hookrightarrow |\beta_{n + 1 - m}| < \frac{\eps}{2M_1}
		\]
	...
	В итоге имеем:
	\[
		(\forall \eps > 0)\ (\exists N \in \N) : (\forall n > N)\ |\gamma_n| < \eps \llra \liml_{n \to \infty} \gamma_n = 0
 	\]
\end{proof}
...
\begin{definition}
	...
	\[
		(\forall \eps > 0)\ (\exists N \in \N) : (\forall n > N) (x \in E) \hookrightarrow |f_n(x) - f(x)| < \eps
	\]
	И \textit{сходится поточечно} на $E$ к функции $f$, если
	\[
		(\forall x \in E)\ (\forall \eps > 0)\ (\exists N \in \N) : (\forall n > N) \emb |f_n(x) - f(x)| < \eps
	\]
\end{definition}
...
\begin{corollary}
	...
	\[
		\exists \{x_n\}_{n = 1}^\infty \subset E : |f_n(x_n) - f(x_n)| \centernot{\xrightarrow[n \to \infty]{}} 0
	\]
\end{corollary}
...
\begin{theorem} (Критерий Коши равномерной сходимости функциональных последовательностей)
	...
	\[
		(\forall \eps > 0)\ (\exists N \in \N) : (\forall n > N, p \in \N, x \in E) \emb |f_{n + p}(x) - f_n(x)| < \eps
	\]
\end{theorem}
...
\begin{proof}~
	\begin{itemize}
		...
		\[
			(\forall \eps > 0)\ (\exists N \in \N) : (\forall n > N, x \in E) \emb |f_n(x) - f(x)| < \frac{\eps}{2}
		\]
		...
		\item Достаточность. Раз условие Коши выполнено (здесь имеется в виду критерий Коши для числовой последовательности), то
		\[
			(\forall x \in E)\ (\exists f(x)) : \liml_{n \to \infty} f_n(x) = f(x)
		\]
...
\begin{theorem} (Критерий Коши равномерно сходимости функциональных рядов)
	...
	\[
		(\forall \eps > 0)\ (\exists N \in \N) : (\forall n > N, p \in \N, x \in E) \emb \left|\suml_{k = n + 1}^{n + p} f_n(x)\right| < \eps
	\]
\end{theorem}
...
\begin{proof}
	Действительно, положим $p = 1$ в критерие Коши и получим требуемое.
\end{proof}

\begin{theorem} (Признак Вейерштрасса)
	Если ряд $\row{n = 1}{c_n}$ - сходящийся и верно, что
	\[
		\forall x \in E, n \in \N \quad |f_n(x)| \le c_n
	\]
	 то $\row{n = 1}{f_n(x)}$ равномерно сходится на $E$.
\end{theorem}
...
\begin{definition}
	...
	\[
		(\exists M \ge 0) : (\forall x \in E, n \in \N) \emb |f_n(x)| \le M
	\]
\end{definition}
...
\begin{theorem} (Признак Дирихле равномерной сходимости функциональных рядов)
	...
	\begin{enumerate}
		\item Последовательность частичных сумм ряда $\row{n = 1}{a_n(x)}$ равномерно ограничена на $E$
...
\begin{proof}
	...
	\[
		(\forall k \in \N, x \in E)\emb |A_k(x)| \le 2M
	\]
	Значит
	\[
	...
	\[
		(\forall \eps > 0)\ (\exists N \in \N) : (\forall n > N, x \in E) \emb |b_n(x)| < \frac{\eps}{8M}
	\]
	Отсюда
	\[
...
\begin{theorem} (Признак Абеля равномерной сходимости функциональных рядов)
	...
	\begin{enumerate}
		\item Ряд $\row{n = 1}{a_n(x)}$ равномерно сходится на $E$
		
		\item $\forall x \in E\ $ последовательность $\ \{b_n(x)\}_{n = 1}^\infty$ монотонна и равномерно ограничена на $E$
	\end{enumerate}
...
\begin{proof}
	...
	\[
		(\forall k \in \N, x \in E)\emb |b_k(x)| \le M
	\]
	А для сумм $A_k(x)$ из критерия Коши верно, что
	\[
		(\forall \eps > 0)\ (\exists N \in \N) : (\forall n > N, x \in E) \emb |A_k| < \frac{\eps}{4M}
	\]

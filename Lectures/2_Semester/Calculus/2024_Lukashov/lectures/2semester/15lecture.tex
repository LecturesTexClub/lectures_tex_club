...
\begin{proof}
	...
	\]
	Данная функция очевидным образом определена и непрерывна, так как ряд
	\[
		\left|\left(\frac{3}{4}\right)^n \phi(4^n x)\right| \le \left(\frac{3}{4}\right)^n
	\]
	равномерно сходится по признаку Вейерштрасса.
	
	По мнению автора, читателю не стоит себя утруждать попытками представить эту функцию. А вот уделить внимание элегантному доказательству искомого  свойства можно.
	
	Верно следующее утверждение:
	\[
		(\forall x \in \R, m \in \N)\ (\exists k \in \Z) : K \le 4^m x \le K + 1
	\]
	...
	\begin{multline*}
		|f(\beta_m) - f(\alpha_m)| = \left|\row{n = 0}{\left(\frac{3}{4}\right)^n (\phi(4^n\beta_m) - \phi(4^n\alpha_m))}\right| \ge
		\\
	...
	\end{multline*}
	Последний переход получен из того, что слагаемое при $n=m$ самое большое, и минимум будет, если все остальные разности раскроются с противоположным знаком.
	...
	\[
		\frac{f(\beta_m) - f(\alpha_m)}{\beta_m - \alpha_m} = \frac{\beta_m - x}{\beta_m - \alpha_m} \cdot \left(\frac{f(\beta_m) - f(x)}{\beta_m - x} - f'(x)\right) + \frac{x - \alpha_m}{\beta_m - \alpha_m} \cdot \left(\frac{f(x) - f(\alpha_m)}{x - \alpha_m} - f'(x)\right) + f'(x)
	\]
	...
	\begin{align*}
		...
		\\
		&{\frac{f(x) - f(\alpha_m)}{x - \alpha_m} \xrightarrow[m \to \infty]{} f'(x)}
	\end{align*}
	Отсюда
	\[
...
\begin{note}
	...
	Приведём несколько соответствующих утверждений (здесь и далее $z_n \in\Cm$).
	\begin{itemize}
		\item $\sum\limits_{n=0}^\infty z_n$ сходится $\lra\ \{\sum\limits_{n=0}^m z_n\}_{m=0}^\infty$ сходится\\
		\item 	$\sum\limits_{n=0}^\infty z_n$ абсолютно сходится $\lra\sum\limits_{n=0}^\infty |z_n|$ сходится \\
		\item Критерий Коши $(\forall\eps > 0)\ (\exists N\in\N) : (\forall n > N, p\in\N)\emb|\sum\limits_{k=n+1}^{n+p}z_n| <\eps $
		\item Необходимое условие $\sum\limits_{n=0}^\infty z_n$ сходится $\Longrightarrow\liml_{n\to\infty}z_n=0$
		\item Признак Вейерштрасса. Если $|f_n(z)|\le c_n\in\R^+_0\ (\forall z\in K\subset\Cm)(\forall n\in\N)$, и $\sum\limits_{n=0}^\infty c_n$ сходится, то $\sum\limits_{n=0}^\infty f_n(z)$ сходится равномерно
	\end{itemize}
...
\begin{theorem} (Другая формула радиуса сходимости)
	Если $\exists \liml_{n \to \infty} |c_n / c_{n + 1}| = R\in[0, +\infty]$, то ряд $\row{n = 0}{c_n(z - z_0)^n}$ имеет радиус сходимости $R$.
\end{theorem}
...
\begin{proof}
	...
	\[
		(\forall z, |z - z_0| \le r) \emb |c_n(z - z_0)^n| \le |c_n|r^n \Longrightarrow \text{сходится равномерно по признаку Вейерштрасса}
	\]
\end{proof}
...
\begin{proof}
	Т.к. $c_n(z-z_0)^n$ - непрерывна на $\Cm$, то по следствию из теоремы о предельном переходе в равномерно сходящихся последовательностях, $\sum\limits_{n=0}^\infty c_n(z-z_0)^n$ непрерывна на $\{z : |z-z_0|\le r\}\quad (\forall r < R)$. Но т.к. любая $z$ в открытом круге радиуса $R$ лежит и в каком-то открытом круге меньшего радиуса с тем же центром, то в такой $z$ функция непрерывна, т.е. непрерывна во всём круге сходимости. 
\end{proof}

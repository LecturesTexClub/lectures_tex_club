...
\begin{definition}
	...
	\[
		\forall t \in \range{m}\ \forall (\vv{x}, \vv{y}) \in E, i \in \range{n}, j \in \range{m}\ \ps{\exists \pd{F_t}{x_i}(\vv{x}, \vv{y}) \wedge \exists \pd{F_t}{y_j}(\vv{x}, \vv{y})}
	\]
\end{definition}
...
\begin{proof}
	...
	\[
		\forall j \in \range{n}\ F_j(\vv{x}, \vv{y}) = \phi_j(\vv{x}) - y_j
	\]

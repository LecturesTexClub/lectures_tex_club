...
\begin{theorem} \textit{(Формула замены переменной)}
	Если $g, g'\in R[\alpha; \beta], g(\alpha)=a, g(\beta)=b, (\forall x \in [\alpha; \beta]) m\leq g(x)\leq M, f\in C[m,M]$. 
	Тогда 
	\[
...
\begin{theorem} \textit{(Первая теорема о среднем)}
	Пусть $f(x)$ и $g(x)$ интегрируемы по Риману на $[a;b]$, причем $\forall x \in [a;b]\ g(x) \geq 0$, $m:=\inf\limits_{x\in[a,b]}{f(x)}, M:=\sup\limits_{x\in[a,b]}{f(x)}$. Тогда 
	\[
...
\begin{anote}
	А вот тут прикол, Лукашов сказал что в некоторых источниках, следующие за этим замечанием, вторая теорема о среднем и формула Бонне, называются наоборот, даже больше, он сам поменял их названия местами год назад. Так что не удивляйтесь, если встретитесь с инвертированными наименованиями. Кроме того на лекции, лемму он внёс внутрь доказательства теоремы, что по мнению автора уменьшило читабельность, так что он позволит себе слегка изменить порядок изложения в этом месте.
\end{anote}

\begin{lemma} \textit{(Формула Бонне)}
...
\begin{proof}
	...
	\[
		(\forall \eps > 0)\ (\exists \delta_1 > 0) : (\forall P, \Delta P < \delta_1) \hookrightarrow \left|s_j - g(a)\cdot \int_a^bf(x)dx\right| < \frac{\eps}{4},\ j = 2, 3
	\]
	Получаем в итоге утверждение
	\[
		(\forall \eps > 0)\ (\exists \delta_1 > 0) : (\forall P, \Delta P < \delta_1) \hookrightarrow \suml_{k = 1}^n(M_k - m_k)g(x_{k - 1})\Delta x_k < \eps
	\]
	Теперь распишем левый интеграл из теоремы по критерию:
	\[
		(\forall \eps > 0)\ (\exists \delta_2) : (\forall P, \Delta P < \delta_2) \hookrightarrow \left|\suml_{k = 1}^nf(x_{k - 1})g(x_{k - 1}) \Delta x_k - \int_a^bf(x)g(x)dx\right| < \eps
	\]
	...
	\[
		(\forall \eps > 0)\ (\exists \delta = \min \{\delta_1, \delta_2\}) : (\forall P, \Delta P < \delta) \hookrightarrow \left|\suml_{k = 1}^n\mu_kg(x_{k - 1}) \Delta x_k - \int_a^bf(x)g(x)dx\right| < 2\eps
	\]
	Или же
	\[
	...
	\[
		\exists \mu_k \in [m_k; M_k] : \int_{x_{k - 1}}^{x_k} f(x)dx = \mu_k \int_{x_{k - 1}}^{x_k} dx = \mu_k\Delta x_k
	\]
	...
	\[
		\exists \xi \in [a;b] : F(\xi) = \int_a^\xi f(x)dx =  \frac{1}{g(a)}\int_a^bf(x)g(x)dx
	\]
\end{proof}
...
\begin{theorem} \textit{(Формула Тейлора с остаточным членом в интегральной форме)}
	Пусть $f(x)$ вместе со своими производными до порядка $n + 1$ включительно интегрируема по Риману на  $[a-\delta;a+\delta], \delta>0$ .
	Тогда:
	\[
		(\forall x \in [a-\delta;a+\delta]) \quad f(x) = f(a) + \suml_{k = 1}^n\frac{f^k(a)}{k!}(x - a)^k + \frac{1}{n!}\int_a^xf^{(n + 1)}(t)(x - t)^ndt
	\]
\end{theorem}
...
\begin{proof} Проведём индукцию по $n$:
	\begin{itemize}
		...
		Подставив, вместо его левой части, в искомую формулу для $n=m$, правую часть, получим искомую формулу для $n=m+1$.	\end{itemize}
\end{proof}